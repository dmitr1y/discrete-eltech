...
export default class HornerTest extends Component {
    ...
    render() {
        return (
            <div className="content-wrap"><Toolbar/> <h1> Схема Горнера </h1> <h2> Контроль </h2> {this.state.input ?
                <div><p> Поделим многочлен{this.polynomial(this.state.input[0])}на бином
                    ({this.polynomial([1, -1 * this.state.input[1]])})</p>
                    <div className="table"><Table
                        data={this.state.table.map((row, i) => row.map((col, j) => {
                            return i == 0 && j == 0 ? <input type="number" disabled={true}/> :
                                <input type="number"/>
                        }))}/></div>
                    ;
                    <div className="button-wrap">
                        ...
                        {this.state.status !== undefined ? (this.state.status ?
                            <i className="checker ok"></i> : <i className="checker wrong"></i>) : null}
                    </div>
                </div> : null}
            </div>)
    }
