...
export default class DiophantineTrainer extends Component {
    ...
    refreshExample() {
        fetch(access.domain + '/solve/diophantine')
    ...
    render() {
        return (<div className="content-wrap"><Toolbar smartTable={true}/>
            <h1> Решение диофантова уравнения </h1>
            <h2> Тренажёр </h2>{this.state.input ?
                <div><p> Решите уравение: {this.state.input[0]}x + {this.state.input[1]}y
                    = {this.state.input[2]}</p>
                    <div className="answer-area">НОД({this.state.input.join(', ')}) = & nbsp;
                        <div className="input-number-wrap"><input type="number"
                                                                  data-original={this.state.output.nod}
                                                                  onBlur={e => this.check(e)}/>
                            <i className="checker"></i></div>
                    </div>
                    <div className="answer-area">a1 = & nbsp;
                        <div className="input-number-wrap"><input type="number" data-original={this.state.output.a}
                                                                  onBlur={e => this.check(e)}/>
                            <i className="checker"></i>
                        </div> & nbsp; b1 = & nbsp; <div className="input-number-wrap"><input type="number"
                                                                                              data-original={this.state.output.b}
                                                                                              onBlur={e => this.check(e)}/>
                            <i className="checker"> </i></div> & nbsp; c1 = & nbsp; <div className="input-number-wrap">
                            <input type="number" data-original={this.state.output.c} onBlur={e => this.check(e)}/> <i
                            className="checker"> </i></div></div>
                    <p> Где a1, b1, c1 - сокращенные коэффиценты уравнения </p> <Table
                        data={this.state.table.map(row => row.map(col => col !== '' ? (
                            <div className="input-number-wrap"><input type="number" data-original={col}
                                                                      onBlur={e => this.check(e)}/>
                                <i className="checker"></i></div>) : null))}/> <p> Ответ : </p>
                    <div className="output" ref="outputWrap">
                        <div className="answer-area"> X = & nbsp; <div className="input-number-wrap"><input
                            type="number" data-original={this.state.output.x[0]} onBlur={e => this.check(e)} ref="a1"/>
                            <i className="checker"> </i></div> & nbsp; + & nbsp; <div className="input-number-wrap">
                            <input type="number" data-original={this.state.output.x[1]} onBlur={e => this.check(e)}
                                   ref="b1"/> <i className="checker"> </i></div> & nbsp; t
                        </div>
                        <div className="answer-area"> Y = & nbsp; <div className="input-number-wrap"><input
                            type="number" data-original={this.state.output.y[0]} onBlur={e => this.check(e)} ref="a2"/>
                            <i className="checker"> </i></div> & nbsp; + & nbsp; <div className="input-number-wrap">
                            <input type="number" data-original={this.state.output.y[1]} onBlur={e => this.check(e)}
                                   ref="b2"/> <i className="checker"> </i></div> & nbsp; t
                        </div>
                    </div>
