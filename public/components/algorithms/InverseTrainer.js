...
export default class InverseTrainer extends Component {
    ...
    render() {
        return (
            <div className="content-wrap"><Toolbar/><h1> Нахождение обратного числа </h1>
                <h2> Тренажёр </h2>{this.state.input ?
                    <div><p> Найти обратный элемент к{this.state.input[1]}в поле вычетов по модулю{this.state.input[0]}заполнив
                        нужную часть таблицы расширенного алгоритма Евклида </p><Table
                        data={this.state.table.map(row => row.map(col => col !== '' ? (
                            <div className="input-number-wrap"><input type="number" data-original={col}
                                ...
                                <i className="checker"></i></div>) : null))}/> <code className="answer-area"> Ответ : &
                        nbsp; <div className="input-number-wrap"><input type="number" className="output"
