...
export default class ConvergentsShow extends Component {
    ...
    refreshExample() {
        fetch('https://edu.konstantinov.com.ru/app/evklid/solve/convergents')
            ...
            <div>
                {this.state.input ?
                    <div className="content-wrap">
                        ...
                        <p> Возьмем {this.state.input[0]} и {this.state.input[1]} и применим расширенный
                            алгоритм Евклида для подходящихдробей.
                        </p>
                        <p> Получим следующую таблицу:
                        </p>
                        <Table data={this.state.table.map((row, i) => row.map((col, j) =>
                            <div className={'number-wrap' + (i == 2 && j > 1 ? ' primary-answer' : '')
                            + (i == 3 && j > 1 ? ' secondary-answer' : '')}>
                                {col}
                            </div>
                        ))
                        }
                        />
                        <code> Ответ : [{output.map(f => f[0] + '/' + f[1]).join(', ')}]
                        </code>
