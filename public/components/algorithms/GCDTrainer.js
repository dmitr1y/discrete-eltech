            ...
            <div className="content-wrap"><Toolbar/><h1> Наибольший общий делитель </h1>
                <h2> Тренажёр </h2>{this.state.input ?
                    <div><p> Даны числа{this.state.input.join(', ')}.</p><p> Примените к ним алгоритм Евклида(для
                        ...
                        значения в следующую таблицу:</p><Table
                        data={this.state.table.map(row => row.map(col => col !== '' ? (
                            <div className="input-number-wrap"><input type="number" data-original={col}
                                ...
                                <i className="checker"></i></div>) : ''))}/> <code
                        className="answer-area"> НОД({this.state.input.join(', ')}) = & nbsp; <div
                        className="input-number-wrap"><input type="number" data-original={this.state.output}
                                                             onBlur={e => this.check(e)}/> <i className="checker"> </i>
                    </div></code>
